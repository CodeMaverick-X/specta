...
export default function LogIN({ open, handleClose, authFormCheck }: { open: boolean, handleClose: () => void, authFormCheck: [AuthFormType, Dispatch<SetStateAction<AuthFormType>>]; }) {

    const [authForm, setAuthForm] = authFormCheck;
    if (authForm === 'signup') return;
    ...
    return (
        <Modal onClose={handleClose} isOpen={open}>
            ...
            >
                <div>
                    <div className=" text-3xl text-[#5BE94F]">Glad to have you back at Specta!</div>
                ...
                <div className=" flex flex-col  pr-16  w-2/5 space-y-0">
                    ...
                    <div className="flex flex-row justify-between pt-5">
                        ...
                        <div className=" text-gray-500">don't have an account? <span onClick={handleFormChange} className="text-[#5BE94F] hover:underline hover:cursor-pointer">signup</span> </div>
                        <button className="inline text-gray-300 my-3 bg-[#1D5F1B] h-10 w-24 p-1 rounded-sm border-[#5BE94F] border hover:bg-[#5BE94F] hover:text-black" type="submit">Log In</button>
                    </div>
